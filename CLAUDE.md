...
- `GEMINI_API_KEY` must be set as a Supabase Edge Function secret (Project Settings → Edge Functions → `thesis-chat` → Secrets, or `supabase secrets set GEMINI_API_KEY=...`) after the 2026-07-29 switch away from the Lovable AI Gateway — without it, `thesis-chat` fails on every request. Verify with a direct curl to the deployed function before assuming the chatbot works in prod.
